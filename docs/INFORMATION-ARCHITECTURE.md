...
| Major sections  | Hero (headline, search trigger, city trigger, join); six service categories; popular services (mapped tiles only); drawing consultation (mapped tiles only); optional banner carousel; platform intro + why-us; join CTA; optional knowledge preview; FAQ entry to `/faq` or category picker; **omit** expert showcase and home testimonials until confirmed |
